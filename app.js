...
//   Building puzzles out of galaxies
// ========================================
...
//              Phuzzle by Carson Design & Prototyping | CAR-NOVA.i 
...
function syncModsUI(){
  ...
  if (modFog) {
    modFog.checked = !!modifierState.active.fog;
    modFog.disabled = locked;
  }
...
function fogKickAnim() {
  if (fogState.raf) return;
  fogState.lastT = performance.now();

  const tick = () => {
    fogState.raf = 0;
    if (!fogIsEnabled()) return;

    const now = performance.now();
    const dt = Math.max(0, now - (fogState.lastT || now));
    fogState.lastT = now;

    // Smooth approach: faster when turning on, slower fade when turning off.
    const turningOn = fogState.targetAlpha > fogState.revealAlpha;
    const lambda = turningOn ? 18 : 5; // bigger = snappier
    const k = 1 - Math.exp(-(lambda * dt) / 1000);

    fogState.revealAlpha = fogState.revealAlpha + (fogState.targetAlpha - fogState.revealAlpha) * k;

    // Keep animating if we're still easing alpha OR if solve bloom/fade is active.
...
function fogOnSolved() {
  if (!fogIsEnabled()) return;

  const now = performance.now();
  fogState.solveBloom = { t0: now, dur: 650 };
  fogState.solveFade  = { t0: now, dur: 900 };

  // Center the reveal for the victory moment
  const size = canvas.width / dpr;
  fogState.x = size * 0.5;
  fogState.y = size * 0.5;

  fogState.targetAlpha = 1;
  fogState.revealAlpha = Math.max(fogState.revealAlpha || 0, 0.75);
...
modFog?.addEventListener("change", () => {
  if (modifierState.locked) {
    syncModsUI();
    return;
  }

  modifierState.active.fog = !!modFog.checked;

  // Fog is purely a render overlay, so no shuffle side effects needed.
  // Give desktop an immediate reveal position for nicer first impression.
  if (modifierState.active.fog) {
    const center = { x: (canvas.width / dpr) / 2, y: (canvas.height / dpr) / 2 };
    fogState.x = lastPointerCanvas?.x ?? center.x;
    fogState.y = lastPointerCanvas?.y ?? center.y;
    fogState.targetAlpha = 1;
...
function resetTimerAndMoves() {
  ...
  fogState.clearedAfterSolve = false;
  fogState.revealAlpha = 0;
  fogState.targetAlpha = 0;
  fogState.solveBloom = null;
  fogState.solveFade = null;
  updateStats();
}
...
function drawFogOfWar(size) {
  ...
  if (fogState.solveBloom) {
    ...
    r = baseRadius * (1 + 0.55 * ease);           // up to +55% radius
    fogAlpha = baseFogAlpha * (1 - 0.22 * ease);  // up to -22% fog density
    tintBoost = 0.10 * (1 - k);                   // quick purple hit that decays

    if (t >= 1) fogState.solveBloom = null;
  }

  // Fade: gently fade fog away after solve
  if (fogState.solveFade) {
    const t = (now - fogState.solveFade.t0) / fogState.solveFade.dur;
    const k = Math.max(0, Math.min(1, t));
    const ease = 1 - Math.pow(1 - k, 2); // smoother

    fogAlpha = fogAlpha * (1 - 0.85 * ease); // approaches near-clear
  ...
  // Base fog fill (gray-blue)
  fogCtx.fillStyle = `rgba(18,22,28,${fogAlpha})`;
  fogCtx.fillRect(0, 0, size, size);

  // Subtle fog haze to avoid flat blackout
  const haze = fogCtx.createRadialGradient(
    size * 0.5, size * 0.5, 0,
    size * 0.5, size * 0.5, size * 0.75
  );
  haze.addColorStop(0, "rgba(255,255,255,0.06)");
  haze.addColorStop(1, "rgba(255,255,255,0)");
  fogCtx.fillStyle = haze;
  fogCtx.fillRect(0, 0, size, size);

  // State tint (only add purple when locked/live)
  if (modifierState.locked) {
    fogCtx.fillStyle = `rgba(160,80,255,${0.05 + tintBoost})`;
    fogCtx.fillRect(0, 0, size, size);
  } else {
    fogCtx.fillStyle = "rgba(255,255,255,0.06)";
    fogCtx.fillRect(0, 0, size, size);
  }

  // Edge vignette so corners feel heavier
  const edge = fogCtx.createRadialGradient(
  ...
  // Composite fog layer onto the main scene last
  ctx.save();
...
if (splashPill) splashPill.hidden = true;
  ...
  // 9) Fog of War (must be last to mask everything)
  drawFogOfWar(size);
}
...
function beginDrag(e) {
  ...
  // Fog of War: on mobile, reveal follows finger while touching.
...
function moveDrag(e) {
  // Fog of War: keep reveal pinned to finger while touching.
  if (fogIsEnabled() && e.pointerType === "touch") {
...
canvas.addEventListener(
  ...
  (e) => {
    ...
    // Fog of War: desktop reveal follows the cursor at all times.
    if (fogIsEnabled() && e.pointerType !== "touch") {
      fogState.x = lastPointerCanvas.x;
      fogState.y = lastPointerCanvas.y;
      fogState.isTouchActive = false;
      fogState.targetAlpha = 1;
      fogState.revealAlpha = 1; // keep it crisp on desktop
      requestDraw();
    }
  },
  { passive: true }
